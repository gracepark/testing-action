...
**Note:** workflows using `required: true` will not automatically return an error if the input is not specified for events that automatically trigger workflow runs. If you set `required: true` in your workflow file and are using `workflow_dispatch` to manually run the workflow, you will be required to specify inputs on GitHub.com. For more information, see "[AUTOTITLE](/actions/using-workflows/events-that-trigger-workflows)."
...
For more information about using the `CMD` instruction with GitHub Actions, see "[AUTOTITLE](/actions/creating-actions/dockerfile-support-for-github-actions#cmd)."
